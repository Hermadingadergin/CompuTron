    ...
    do {
        ...
        *opPtr = *irPtr % 100; //operand = instructionRegister % 100; // remainder
        ...
        case Command::divide:
            ...
            word = *acPtr / memory[*opPtr]; // Divide the value in the accumulator (acPtr) by the value in memory at the location pointed to by 'opPtr' and store the result in 'word'
            if (validWord(word)) // If the result is valid, store it in the accumulator and increment the instruction counter
            {
                *acPtr = word;
                (*icPtr)++;
